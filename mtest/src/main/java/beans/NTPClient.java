 ...
 * ...) See <A HREF="ftp://ftp.rfc-editor.org/in-notes/rfc868.txt"> the
 * spec </A> for details.
 * <p>
...
public final class NTPClient
{
  ...
  public static void main(String[] args)
  {
     ...
     * "Usage: NTPClient <hostname-or-address-list>"); System.exit(1); }
     * 
     * NTPUDPClient client = new NTPUDPClient(); // We want to timeout if a
     * response takes longer than 10 seconds client.setDefaultTimeout(10000);
     * try { client.open(); for (String arg : args) { System.out.println(); try
     * { InetAddress hostAddr = InetAddress.getByName(arg); System.out.println(
     * "> " + hostAddr.getHostName() + "/" + hostAddr.getHostAddress());
     * TimeInfo info = client.getTime(hostAddr); processResponse(info); } catch
     * (IOException ioe) { ioe.printStackTrace(); } } } catch (SocketException
     * e) { e.printStackTrace(); }
     * 
     * client.close();
     */
    ...
    // TimeStamp timestamp = new Network().getNtpRefTime(new String[] {
    // "pool.ntp.org", "us.pool.ntp.org" });
    // System.out.println("------------------------\n" + timestamp + "\n" +
    // timestamp.getTime() + "\n" + timestamp.toUTCString() + "\n" +
    // timestamp.getDate()
    // + "\n" + new Date(timestamp.getTime()) + "\nformated:" +
    // sdf.format(timestamp.getDate()) + "\n"
